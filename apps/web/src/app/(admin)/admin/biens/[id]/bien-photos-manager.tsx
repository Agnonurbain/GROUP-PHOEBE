...
export default function BienPhotosManager({
  bienId,
  photos,
}: {
  bienId: string;
  ...
  const [uploadState, uploadAction] = useActionState<BienState, FormData>(
    ajouterPhotosBien,
    {}
  );
  ...
  const handleDelete = (mediaId: string) => {
    ...
    startTransition(async () => {
      await supprimerPhotoBien(mediaId);
    });
  };

  const handleMove = (mediaId: string, direction: "up" | "down") => {
    startTransition(async () => {
      await reordonnerPhotoBien(mediaId, direction);
    });
  };
