    ...
    <form action={action} className="space-y-5">
      ...
      <fieldset className="rounded-2xl border border-public-border bg-public-bg-card p-5">
        <legend className="px-1 text-sm font-semibold text-public-text">
          {t.textile.quelPagne}<Obligatoire />
        </legend>
      ...
      <div className="grid gap-4 rounded-2xl border border-public-border bg-public-bg-card p-5 sm:grid-cols-2">
        <div>
          <label htmlFor="quantite" className={label}>{t.textile.quantite}<Obligatoire /></label>
          <input
            id="quantite"
            name="quantite"
            type="number"
        ...
        <div className="sm:col-span-2">
          ...
          {!article && (
            <p className="mt-1 text-[11px] text-public-text-faint">
              {t.textile.rienAuCatalogue}
            </p>
          )}
        </div>
        <div className="sm:col-span-2">
          <label htmlFor="couleurs" className={label}>{t.textile.couleursSouhaitees}</label>
          <input
            id="couleurs"
            name="couleurs"
            maxLength={200}
            placeholder={t.textile.exempleCouleurs}
            className={champ}
          />
        </div>
        <div className="sm:col-span-2">
          <label htmlFor="message" className={label}>{t.textile.precisions}</label>
          <textarea
            id="message"
            name="message"
            rows={2}
            placeholder={t.textile.examplePrecisions}
            className={champ}
          />
        </div>
      </div>
      ...
      >
        {pending ? t.commun.envoi : t.textile.demanderDevis}
      ...
      <p className="rounded-xl border border-accent-gold/25 bg-accent-gold/5 p-4 text-xs leading-relaxed text-public-text-muted">
        <strong className="text-public-text">{t.textile.pourquoiPasDePrix}</strong>{" "}
        {t.textile.pourquoiPasDePrixTexte}
      </p>
    </form>
  )
}
