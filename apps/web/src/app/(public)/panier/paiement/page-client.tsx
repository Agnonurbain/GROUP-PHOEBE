...
import { pluriel, remplir } from "@/lib/i18n/format"
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-6xl px-6 pt-6 sm:px-10">
        <BackLink href="/panier" label={t.paiement.retourPanier} />
      </div>
          ...
          <Card>
            ...
            {nbJours > 0 && (
              <p className="mt-3 text-xs text-public-text-muted">
                {pluriel(langue, { un: t.paiement.joursLocation_un, autre: t.paiement.joursLocation_pluriel }, nbJours)}
              </p>
            )}
          </Card>
          ...
          <Card>
            ...
            <div className="relative">
              <label htmlFor="destination" className="block text-sm font-medium text-public-text-muted mb-1.5">
                {t.paiement.villeLivraison}<Obligatoire />
              </label>
              <input
                ref={inputRef}
                id="destination"
          ...
          <Card className="sticky top-24">
            ...
            {chauffeurObligatoire && (
              <p className="mt-2 text-xs text-accent-orange">
                {t.paiement.chauffeurObligatoire}
              </p>
            )}
            {detectedZone && (
            ...
            <div className="mt-6 flex flex-wrap gap-2 text-xs text-public-text-muted">
              ...
              <span className="flex items-center gap-1 rounded-md bg-public-bg-elevated px-2 py-1">
                ...
                {t.paiement.annulationGratuite}
              </span>
              <span className="flex items-center gap-1 rounded-md bg-public-bg-elevated px-2 py-1">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                {t.paiement.support247}
              </span>
            </div>
              ...
              <div className="mt-3 space-y-3">
                ...
                <div>
                  <label htmlFor="conducteur_secondaire_permis" className="mb-1 block text-xs font-medium text-public-text">
                    {t.paiement.permisSecondConducteur}
                  </label>
                  <input
              ...
              >
                {pending ? (
                  <>
                    ...
                    {t.paiement.traitementEnCours}
                  </>
                ) : (
                  <>{remplir(t.paiement.confirmerEtPayer, { montant: `${totalAvecDuree.toLocaleString(langue === "en" ? "en-US" : "fr-FR")} ${t.commun.devise}` })}</>
                )}
              </Button>
            </div>
