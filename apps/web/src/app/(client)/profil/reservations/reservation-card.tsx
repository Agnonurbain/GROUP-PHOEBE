    ...
    <div className="group rounded-2xl border border-phoebe-pearl bg-white p-6 space-y-4 shadow-sm transition-all duration-300 hover:shadow-xl hover:border-phoebe-gold/20 relative overflow-hidden">
      ...
      {demande.statut === "en_negociation" && !demande.prix_negocie && (
        <div className="rounded-lg bg-phoebe-gold/5 px-4 py-3 text-sm text-phoebe-gold">
          {isAchat
            ? "En cours de négociation — un opérateur vous contactera sur WhatsApp."
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        {["acceptee", "en_cours", "terminee", "retour_en_inspection"].includes(demande.statut) && (
          <a
            href={`/api/contrat-pdf?id=${demande.id}`}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 text-xs text-phoebe-anthracite/50 hover:text-phoebe-green transition-colors"
          >
