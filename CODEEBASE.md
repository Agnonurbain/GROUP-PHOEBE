...
- **Unitaires** : Vitest — 917 tests dans 46 fichiers, dans `apps/web/__tests__/`
...
| Date | Changement |
|---|---|
| 2026-08-06 | **La dette de traduction est soldée : les libellés d'état passent au dictionnaire.** 75 libellés vivaient dans des tables de constantes de `lib/` — statuts d'une demande, modes et zones de livraison, pièces d'un dossier, classes de vol, types de bien. Ce sont des fichiers `.ts`, et la garde ne balayait que le JSX : un client anglophone lisait « En cours de traitement » sur sa demande et « Même jour » dans le choix du mode, au milieu d'une page par ailleurs traduite. **Les tables restent**, en repli français, pour ce qui ne s'affiche pas au visiteur — notifications internes, journal, administration ; ce que la garde interdit désormais, c'est qu'une page publique les lise. Les **clés sont celles de la base** (`statutLivraison.en_transit`), et un test compare clé par clé le dictionnaire aux tables : une clé perdue afficherait la valeur brute. L'accesseur `libelle()` complète `remplir` et `pluriel` : il prend une table du dictionnaire et une valeur venue de la base, et renvoie la valeur brute si la clé est inconnue — comme le faisait le `?? statut` des tables. **Deux collisions de nom** relevées au passage : `t` désignait la variable d'une boucle dans deux composants, et masquait le dictionnaire ; c'est la boucle qui a été renommée, pas le dictionnaire. |
| 2026-08-05 | **Le textile entre dans les images en mouvement.** Le diaporama de l'accueil comptait six diapositives et aucune pour le cinquième service ; il en a sept. Surtout, la page Textile n'avait **pas de fond animé** — transport, livraison et immobilier en ont un depuis toujours, et c'est ce qui la faisait paraître plus plate que les autres. **Deux décisions sur l'image elle-même.** La source fait 701 px de large : elle est montée à 1000, pas à 1400 comme les autres héros. Au-delà, on ajoute du poids sans ajouter de détail — un motif de pagne est dense, il se comprime mal (207 Ko à 1400 px contre 126 Ko à 1000), et l'image passe sous un dégradé sombre de toute façon. Et la seconde photo d'ambiance a été écartée : un panneau « WAX HOLLANDAIS 6 YARDS 62… » y est lisible. Le service n'affiche aucun prix par décision de l'exploitant (00087) — en laisser un entrer par l'image aurait contredit tout le module. |
